...
- (void)loadPillImageBypassingCache:(BOOL)bypass callback:(INCancelErrorBlock)callback
{
	...
	BOOL inCache = NO;
	if (!inCache || bypass) {
		NSString *apiKey = @"HELUUSPMYB";			/// @attention You will need your personal API key for the server to respond
		...
		[loader getWithCallback:^(BOOL userDidCancel, NSString *__autoreleasing errorMessage) {
			if (errorMessage) {
				...
				// got pills matching the ingredient, find our rxcui
					...
					CANCEL_ERROR_CALLBACK_OR_LOG_ERR_STRING(callback, @"No image")		// maybe log no error but userDidCancel, to avoid showing this "error"?
				}
			}
		}];
	}
